        ...
        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
            ...
            @foreach ($students as $student)
                <tr>
                    ...
                    <td>
                    <!-- في حال كان للطالب سجل حضور او غياب -->
                    <!-- شرط للحصول على سجلات الحضور والغياب لعرض الاختيارات  -->
                        @if(isset($student->attendance()->where('attendence_date',date('Y-m-d'))->first()->student_id))

                            <label class="block text-gray-500 font-semibold sm:border-r sm:pr-4">
                                <input name="attendences[{{ $student->id }}]" disabled
                                       {{ $student->attendance()->latest('created_at')->first()->attendence_status == 1 ? 'checked' : '' }}
                            ...
                            <label class="ml-4 block text-gray-500 font-semibold">
                                <input name="attendences[{{ $student->id }}]" disabled
                                       {{ $student->attendance()->latest('created_at')->first()->attendence_status == 0 ? 'checked' : '' }}
