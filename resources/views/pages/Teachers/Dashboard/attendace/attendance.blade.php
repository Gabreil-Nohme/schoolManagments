                    ...
                    <td>
                        ...
                        <label class="block text-gray-500 font-semibold sm:border-r sm:pr-4">
                            <input name="attendences[{{ $student->id }}]"
                                @foreach ($student->attendance() ->where('attendence_date', date('Y-m-d'))->get() as $item)
                        {{ $item->attendence_status == 1 ? 'checked' : '' }} @endforeach
                        ...
                        <label class="ml-4 block text-gray-500 font-semibold">
                            <input name="attendences[{{ $student->id }}]"
                                @foreach ($student->attendance() ->where('attendence_date', date('Y-m-d'))->get() as $item)
                        {{ $item->attendence_status == 0 ? 'checked' : '' }} @endforeach
